...
function renderMappings() {
  ...
  // Eventy
  document.querySelectorAll(".mapName").forEach(el => el.addEventListener("input", e=>{mappings[e.target.dataset.idx].sqlName=e.target.value; generateSQL();}));
  document.querySelectorAll(".mapType").forEach(el => el.addEventListener("change", e=>{mappings[e.target.dataset.idx].type=e.target.value; generateSQL();}));
  document.querySelectorAll(".mapFixed").forEach(el => el.addEventListener("input", e=>{mappings[e.target.dataset.idx].fixedValue=e.target.value; generateSQL();}));
  document.querySelectorAll(".toggleInclude").forEach(el => el.addEventListener("click", e=>{
...
function renderPreview() {
  ...
  headers.forEach(h=>{const th=document.createElement('th');th.className='text-left text-xs px-3 py-2 font-medium text-gray-600';th.innerText=h||'';trh.appendChild(th);});
  previewTable.appendChild(trh);
  for (let r=0;r<rowsToShow;r++){
    const tr=document.createElement('tr');
    const row=parsedRows[r];
    headers.forEach((h,c)=>{const td=document.createElement('td');td.className='text-sm px-3 py-2 border-t border-gray-100';td.innerText=row[c]??'';tr.appendChild(td);});
...
csvFileInput.addEventListener('change', async e=>{
  ...
  updateProgress(); renderMappings(); renderPreview();
});
...
applyHeaderBtn.addEventListener('click', ()=>{
  ...
  mappings = headers.map(h=>({orig:h,sqlName:h,type:'TEXT',fixedValue:'',include:true}));
  renderMappings(); renderPreview(); updateProgress();
});
...
copySqlBtn.addEventListener('click', async ()=>{
  ...
  try{await navigator.clipboard.writeText(sqlOutput.value); alert('SQL zkopírováno do schránky.');}catch{alert('Kopírování selhalo.');}
});
